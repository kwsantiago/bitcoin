...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, const CheckTxInputsRules rules)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        if (coin.IsCoinBase()) {
            if (rules.test(CheckTxInputsRules::NoCoinbaseSpend)) {
                return state.Invalid(TxValidationResult::TX_PREMATURE_SPEND, "bad-txns-coinbase-spend-frozen",
                    strprintf("tried to spend coinbase at depth %d during the coinbase spend freeze", nSpendHeight - coin.nHeight));
            }
