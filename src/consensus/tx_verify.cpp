...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, const CheckTxInputsRules rules, const int coinbase_maturity, const int coinbase_maturity_from_height)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        if (coin.IsCoinBase() &&
            nSpendHeight - coin.nHeight < RequiredCoinbaseMaturity(coin.nHeight, coinbase_maturity, coinbase_maturity_from_height)) {
