...
bool CPubKey::Verify(const uint256 &hash, const std::vector<unsigned char>& vchSig, const unsigned char *hint) const {
    if (!IsValid())
        return false;
    ...
    return secp256k1_ecdsa_verify(secp256k1_context_verify, &sig, hash.begin(), &pubkey, hint);
}
